...
//! `bmp_token` decoded from `assets/t_s.bmp` by an imath-bignum + matrix decode
//! (sign path) that is not yet ported (TASK-0032). So today this test, when run
//! with `--include-ignored`, asserts the HONEST token-pending state end-to-end
//! rather than a fabricated success. When TASK-0032 lands, the body below is
//! replaced
...
//!
//! ## Manual setup (once TASK-0032 unblocks login)
//!
...
/// because a valid sign is not yet possible (TASK-0032). It makes NO network
/// call and fabricates NO response.
///
/// FUTURE (once TASK-0032 lands): replace the body with
...
#[ignore = "live gold-oracle: needs the user's real Tuya account + the bmp_token \
            (TASK-0032); run manually with --ignored --test-threads=1. Today it \
...
fn live_login_then_device_list_finds_scd921() {
    ...
    let material = SigningKeyMaterial {
        ...
        // path is token-pending. TASK-0032's harness will load these from
    ...
    // (goes red) the day someone makes it pretend to succeed without TASK-0032 —
    // which is exactly the negative-feedback property we want.
    match result {
        Err(Error::BmpTokenPending) => {
            // Expected today. When TASK-0032 lands, this arm is replaced by the
