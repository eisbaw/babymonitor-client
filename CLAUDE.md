# CLAUDE.md — Philips Avent Baby Monitor+ RE

**Read first:** [`re/prd.md`](re/prd.md). It carries the goal, scope, hardware, and the
static-analysis-only constraint across sessions and sub-agents.

## What this project is

Reverse-engineering `com.philips.ph.babymonitorplus` (Philips Avent Baby Monitor+, hardware
SCD921/SCD923) to reimplement a full-parity client in **Rust**. Priority: live video/audio
stream + pairing/auth first.
...
- All RE artifacts under `re/`. Secrets/PII/captures under `secrets/` (gitignored). Never commit
  the APK or decompiled output.

## Toolchain

- Use the nix shell for every tool: `nix-shell --run 'jadx ...'` (apkeep, jadx, apktool,
  radare2, ghidra, ripgrep, cargo, etc. are provided by `shell.nix`).
- Build/test/lint/run live in the `Justfile` once code exists. Run `just e2e` before every commit.

## Backlog workflow

- `backlog` CLI (init with `--cli --claude --leading-zeros 4`). Always use `--plain` for
  `backlog task view` / `backlog board` (TUI hangs non-interactively).
- One task per sub-agent, sequential. Sub-agent reads this file + PRD, assigns the task, sets
  In Progress, implements, checks ACs, creates new tasks for tangents.
- Git commit after each logical task. Do not credit Claude in commit messages. Do not override
  git user/email.

## Phase gates

At each milestone gate, run `mped-architect` (architecture review) and `qa-test-runner`
(`just e2e`) in parallel once there is code/tests; fix findings before advancing. User confirms
each phase advance.
