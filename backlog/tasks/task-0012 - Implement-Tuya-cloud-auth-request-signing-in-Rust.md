...
from TASK-0041 review (P2): the signed-param whitelist uses the WIRE name 'clientId' (ThingApiParams.KEY_APP_ID="clientId"), NOT 'appId' — the Rust canonical-string builder must emit clientId or the sign is wrong. Also: the bmp_token decode's config byte[] = the appKey bytes (mAppId.getBytes()) = static; all signer inputs are now static (see re/bmp_token_provenance.md).
<!-- SECTION:NOTES:END -->
